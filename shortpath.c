...
void DrawMap(newn n,ids s,LGraph Graph){
	...
	for(int j = 2119; j < 3941; j++){
		fprintf(fp,"%lf %lf \n\n\n\n",get_lon(n,Graph->G[j].Data),get_lat(n,Graph->G[j].Data));
	}
	fclose(fp);
}
...
void PrintOut_1(newn n,Vertex E,Table T,FILE *fp){
	...
	printf("%i  ",returnid(n,E));
	fprintf(fp,"%lf %lf \n",get_lon(n,E),get_lat(n,E));
}

void ViewGraph(LGraph Graph){
	printf("View the information of the points which having roads: \n");
	for(int i = 0; i < 3941; i++){
		printf("NO.%i : %i            Coordinate: ( %lf, %lf ) \n",i+1,Graph->G[i].Data,Graph->G[i].lon,Graph->G[i].lat);	
	}
